...
with
    ...
    agg_dip as (
        ...
        -- Indicateurs
            ...
            cible_visee,
        -- Agg
            count(fiche) as nb_resultat,
            CAST(SUM(is_qualified) as integer) as nb_is_qualified,
            CAST(ROUND(AVG(is_qualified), 3) AS FLOAT) AS taux_qualification_fpt,
            CAST(ROUND(AVG(is_qualified) - cible, 3) AS FLOAT) * 100 AS ecart_cible
        from _filtre
        group by
            annee_scolaire,
            id_indicateur_css,
...
-- Agg
    ...
    CONCAT(
        ...
        CHAR(10),
        '(', nb_is_qualified, '/', nb_resultat, ' él.) '
    ) AS taux_nbEleve,
    case
