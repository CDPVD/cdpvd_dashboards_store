...
with
    source as (
        ...
        inner join
            {{ ref("cdpvd_stg_main_job_history") }} as mnj  -- To keep only main jobs
    ...
    expanded as (
        select
            case
                when month(valid_from) between {{ var("mois_reference") }} and 12
        ...
        from
            (
                select
                    ...
                    convert(
                        date,
                        concat(school_year, '-',{{ var("mois_reference") }}, '-01'),
                        102
                    ) as school_year_start,
                    convert(
                        date,
                        concat(
                            school_year + 1, '-',{{ var("mois_reference") }} - 1, '-30'
                        ),
                        102
                    ) as school_year_end,
                    matr,
