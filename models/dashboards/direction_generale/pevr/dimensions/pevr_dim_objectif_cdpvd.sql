...
#}
{{ config(alias="pevr_dim_objectif_cdpvd") }}
...
{%- set source_relation_core = adapter.get_relation(
    ...
    identifier="dim_indicateurs_pevr",
) -%}
--
...
{% if table_exists_css %}
    ...
    -- La seed css et la dimension du core existe.
        {% if execute %}
            {{
                log(
                    "✅  La seed css '*_dashboard_pevr_seeds.custom_indicateurs_pevr_css' existe ET la dimension core 'dim_indicateurs_pevr' existe.
                    Une union va donc être fait entre la seed css 'custom_indicateurs_pevr_css' et la dimension core 'dim_indicateurs_pevr' pour créer
                    la dimension cdpvd 'pevr_dim_objectif_cdpvd'.",
                    true,
                )
            }}
        {% endif %}
    ...
    -- La seed css existe, mais pas la dimension du core.
        {% if execute %}
            {{
                log(
                    "⚠️  La seed '*_dashboard_pevr_seeds.custom_indicateurs_pevr_css' existe mais la dimension 'dim_indicateurs_pevr' n'existe pas.
                    Par défaut, la dimension 'pevr_dim_objectif_cdpvd' va prendre les informations de la seed 'custom_indicateurs_pevr_css'.",
                    true,
                )
            }}
        {% endif %}
...
{% else %}
    ...
    -- La seed CSS n'existe pas, mais la dimension du core existe
        {% if execute %}
            {{
                log(
                    "⚠️  La seed '*_dashboard_pevr_seeds.custom_indicateurs_pevr_css' n'existe pas, mais la dimension 'dim_indicateurs_pevr' existe. Une union va donc avoir lieu entre la seed cdpvd 'cibles_indicateurs_annuelles_pevr_cdpvd' et la dimension core 'dim_indicateurs_pevr' pour créer la dimension cdpvd 'pevr_dim_objectif_cdpvd'",
                    true,
                )
            }}
        {% endif %}
