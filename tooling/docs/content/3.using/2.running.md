...
**BEFORE** calling a DBT command from `cssXX.dashboards_store`, make sure you have activated the Poetry environement by running `eval $(poetry env activate) && poetry install` from `cdpvd_dashboards_store`.
::
...
::alert{type=info}
Please consult `cdpvd_dashboards_store/dbt_project.yaml` to find the tag of the model you want to run
::
