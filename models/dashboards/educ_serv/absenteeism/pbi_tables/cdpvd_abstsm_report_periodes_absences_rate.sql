...
#}
{{ config(alias="cdpvd_report_periodes_absences_rate") }}
...
with
    source as (
        ...
        from {{ ref("cdpvd_abstsm_stg_periodes_metrics") }} as src
        group by annee,  rollup (school_friendly_name, groupe, code_matiere), date_evenement, jour_semaine, event_kind
...
select
    ...
    {{
        dbt_utils.generate_surrogate_key(
            ["annee", "school_friendly_name", "event_kind", "groupe", "code_matiere", "jour_semaine"]
        )
    }} as filter_key,
