...
with
    ...
    age as (
        select
            ...
            lieu_trav,
            motif_abs,
            DATEDIFF(year, emp.birth_date, annee) AS age,
            case
                when DATEDIFF(year, emp.birth_date, annee) < 25
                then '24 ans et moins'
                when DATEDIFF(year, emp.birth_date, annee) >= 25 and DATEDIFF(year, emp.birth_date, annee) < 35
                then '25 à 34 ans'
                when DATEDIFF(year, emp.birth_date, annee) >= 35 and DATEDIFF(year, emp.birth_date, annee) < 45
                then '35 à 44 ans'
                when DATEDIFF(year, emp.birth_date, annee) >= 45 and DATEDIFF(year, emp.birth_date, annee) < 55
                then '45 à 54 ans'
                when DATEDIFF(year, emp.birth_date, annee) >= 55 and DATEDIFF(year, emp.birth_date, annee) < 65
                then '55 à 64 ans'
                when DATEDIFF(year, emp.birth_date, annee) >= 65
                then '65 ans et plus'
            end as 'tranche_age'
...
inner join
    {{ ref("fact_nbr_jours_travailles") }} as cal
