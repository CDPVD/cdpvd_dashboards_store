...
with
    ...
    adr as (
        ...
        where ele.code_perm is not null and adr.adr_eleve = '1'  -- adresse principal uniquement
        ...
        left join {{ ref("i_e_adr") }} adr on adr.fiche = ele.fiche
        where ele.code_perm is not null and adr.ind_envoi_meq = '1'  -- adresse principal uniquement
        union all
        ...
        select
            ele.code_perm,
            case
                when charindex('_', adr.fiche) > 0
                then
                    right('0000000' + left(adr.fiche, charindex('_', adr.fiche) - 1), 7)
        ...
        left join {{ ref("i_e_adr_adultes") }} adr on adr.fiche = ele.fiche
        where ele.code_perm is not null and adr.envoimeq = '1'  -- adresse principal uniquement
        ...
        from
            (
                select
                    *,
                    row_number() over (
                        partition by code_perm order by date_maj desc
                    ) as seq_id
                from adr
            ) as seq
        where seq_id = 1
