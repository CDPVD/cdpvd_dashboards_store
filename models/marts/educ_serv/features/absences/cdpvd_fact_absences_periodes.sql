...
{# {{ 
    config( 
        post_hook=[ 
            core_dashboards_store.create_clustered_index( 
                "{{ this }}", ["fiche", "id_eco", "school_year", "code_matiere", "groupe"] 
            )
        ] 
    ) 
}}  #}
...
with
    src as (
        ...
        where event_kind = 'absence (période)' and code_matiere != 'Tout'
        group by date_abs, fiche, id_eco, groupe, event_kind, code_matiere  -- Superseed is_absence
