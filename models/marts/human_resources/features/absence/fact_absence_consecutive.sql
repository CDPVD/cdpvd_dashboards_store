...
with
    absence as (
        ...
        cal.bal_jour_ouv, -- Jour de l'année scolaire
        ta.categories
...
abd as ( select 
        ...
        ac.categories,
        dure,
    SUM(CASE WHEN cal.jour_sem = 1 THEN ((ac.pourc_sal * dure ) /100 ) / 7 ELSE 0 END) AS lundi,
    SUM(CASE WHEN cal.jour_sem = 2 THEN ((ac.pourc_sal * dure ) /100 ) / 7 ELSE 0 END) AS mardi,
    SUM(CASE WHEN cal.jour_sem = 3 THEN ((ac.pourc_sal * dure ) /100 ) / 7 ELSE 0 END) AS mercredi,
    SUM(CASE WHEN cal.jour_sem = 4 THEN ((ac.pourc_sal * dure ) /100 ) / 7 ELSE 0 END) AS jeudi,
    SUM(CASE WHEN cal.jour_sem = 5 THEN ((ac.pourc_sal * dure ) /100 ) / 7 ELSE 0 END) AS vendredi,
