    ...
    config(
        ...
        post_hook=[
            core_dashboards_store.create_clustered_index(
                "{{ this }}", ["matr", "demission_date", "ref_empl"], unique=True
            ),
        ],
    )
}}
...
with
    demission as (
        select
            ...
            stg.school_year
        from {{ ref("cdpvd_stg_activity_history") }} as stg

        inner join
            (
                select etat_empl, school_year
                from {{ ref("cdpvd_dim_employment_status_yearly") }}
                where empl_resi = 1
            ) as dim
    ...
    add_first_date as (
        select
            ...
            (
                ...
                from {{ ref("cdpvd_stg_activity_history") }} stg_first
