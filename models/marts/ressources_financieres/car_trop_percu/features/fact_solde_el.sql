...
#}
{{ config(tags=["rf", "car_trop_percu"], schema="mart_rf") }}
...
with
    ...
    soldes_gpi as (
        select
            ...
            eco.eco,
            sum(
                case when f.motif_fact not in ('A', 'D', 'R') then f.solde else 0 end
            ) as car_gpi,
    ...
    car_tp_proc as (
        ...
        left join
            ...
            on pop.code_perm = car_tp_proc.code_perm
            and pop.fiche = car_tp_proc.fiche
            and pop.annee = car_tp_proc.annee
        left join
...
left join
    soldes_gpi as gpi
    on gpi.code_perm = perim.code_perm
    and gpi.annee = perim.annee
    and gpi.eco = perim.eco
left join
    car_ag
    on car_ag.code_perm = perim.code_perm
    and car_ag.annee = perim.annee
    and car_ag.eco = perim.eco
left join
    tp_ag
    on tp_ag.code_perm = perim.code_perm
    and tp_ag.annee = perim.annee
    and tp_ag.eco = perim.eco
left join
    car_tp_proc
    on car_tp_proc.code_perm = perim.code_perm
    and car_tp_proc.annee = perim.annee
    and car_tp_proc.eco = perim.eco
