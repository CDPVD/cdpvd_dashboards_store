...
Les bases de données `GPI`, `Procure`, `Jade_adultes` et `Avant-garde` doivent être liées au projet. Veuillez consulter la section [liaison d'une base de données](/3.using/3.configuration/2.databases) pour plus d'informations sur comment lier une base de données.
