...
-- historique des emplois à considérer
    ...
    select distinct
        ...
        case 
            ...
            when left(phe.corp_empl, 1) = '2' then ptce.nb_hres_an / 260.9
...
-- Calculer le nbre d'heures remunerées
    ...
    select 
        ...
        , pmnt.mode
        , case 
			when pmnt.lieu_trav is null then perim.lieu_trav 
			else pmnt.lieu_trav 
		end as lieu_trav
		...
		, chq.no_per
        , case 
            when pmnt.mode in ('1','5','J') then perim.hntj * pmnt.nb_unit 
            ...
            when pmnt.mode = '3' then pmnt.nb_unit * (perim.hntj / 2.0) 
            when pmnt.mode = '4' then pmnt.nb_unit * (perim.hntj * 3.0 / 4.0) 
        end as nombre_heures_remun
    from prd
    left join {{ ref('i_pai_hchq') }} as chq
        on chq.gr_paie = prd.gr_paie and chq.an_budg = prd.an_budg and chq.no_per = prd.no_per and chq.etat_cheq = 3
    left join {{ ref('i_pai_hchq_pmnt') }} as pmnt 
        on pmnt.matr = chq.matr and pmnt.no_cheq = chq.no_cheq and pmnt.date_cheq = chq.date_cheq  
    join perim on perim.matr = pmnt.matr and perim.ref_empl = pmnt.ref_empl and pmnt.date_deb between perim.date_eff and perim.date_fin 
    join (
            select 
                code_pmnt
				, case 
