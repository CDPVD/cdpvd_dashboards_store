...
{{
    config(
     ...
     SELECT pop.code_perm
          , pop.fiche
          , freq.client
          , pop.population
          , pop.annee
          , pop.freq
          , floor(datediff(day, convert(date, el.date_naissance, 112) , cast(concat(pop.annee, '0630') AS date)) / 365.25) AS age_30_juin
            /* format 112 = yyyymmdd */
          , floor(datediff(day, convert(date, el.date_naissance, 112) , cast(concat(pop.annee, '0930') AS date)) / 365.25) AS age_30_septembre
            /* format 112 = yyyymmdd */
          , freq.org
          , freq.eco_cen
          , freq.bat
       ...
       JOIN {{ ref("dim_eleve_adultes") }} AS el
     ON el.code_perm = pop.code_perm
  ...
  LEFT JOIN {{ ref("i_t_wl_descr_adultes") }} wlt
